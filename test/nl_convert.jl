...
facts("[nl_convert] check special conversion cases") do
    ...
    for func in special_cases
        ...
        @fact (eval(JAMSDWriter.convert_formula(expr)) -->
...
facts("[nl_convert] check numeric values") do
    x = rand()
    @fact JAMSDWriter.convert_formula(:($x)) --> :($x)
    x = -rand()
    @fact JAMSDWriter.convert_formula(:($x)) --> :($x)
end
...
facts("[nl_convert] check unary, binary and n-ary plus") do
    expr = :(+(1))
    @fact JAMSDWriter.convert_formula(expr) --> :(1)
    expr = :(1 + 2)
    @fact JAMSDWriter.convert_formula(expr) --> :(1 + 2)
    expr = :(1 + 2 + 3)
    @fact JAMSDWriter.convert_formula(expr) --> :(sum(1, 2, 3))
end

facts("[nl_convert] check unary, binary and n-ary minus") do
    expr = :(- x)
    @fact JAMSDWriter.convert_formula(expr) --> :(neg(x))
    expr = :(x - y)
    @fact JAMSDWriter.convert_formula(expr) --> :(x - y)
    expr = :(x - y - z)
    @fact JAMSDWriter.convert_formula(expr) --> :((x - y) - z)
end
...
facts("[nl_convert] check comparison expansion") do
    expr = :(1 < 2 < 3)
    @fact JAMSDWriter.convert_formula(expr) --> :(1 < 2 && 2 < 3)
    expr = :(1 < 2 < 3 < 4)
    @fact JAMSDWriter.convert_formula(expr) --> :((1 < 2 && 2 < 3) && 3 < 4)
end
