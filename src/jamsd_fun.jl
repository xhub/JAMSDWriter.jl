...
function ctx_getvarval(ctx, idx)
	val = Ref{Cdouble}(NaN)
...
function emp_ovf(emp, name, ovf_vidx, args)
	res = ccall((:ovf_add, "libjamsd"), Cint, (Ptr{empinfo}, Cstring, Cint, Cuint, Ref{Cint}), emp, name, ovf_vidx, sizeof(args), args)
	res != 0 && error("return code $res from JAMSD")
end

function emp_ovf_param(emp, ovf_idx, param_name, scalar::Cdouble)
	res = ccall((:ovf_add_param_scalar, "libjamsd"), Cint, (Ptr{empinfo}, Cuint, Cstring, Cdouble), emp, ovf_vidx, param_name, scalar)
	res != 0 && error("return code $res from JAMSD")
end

function emp_ovf_param(emp, ovf_idx, param_name, arr::Vector{Cdouble})
	res = ccall((:ovf_add_param_vector, "libjamsd"), Cint, (Ptr{empinfo}, Cuint, Cstring, Cuint, Ptr{Cdouble}), emp, ovf_vidx, param_name, sizeof(arr), arr)
	res != 0 && error("return code $res from JAMSD")
end
